...
const BMI_BANDS = [
  { max: 18.5, label: 'Underweight', color: 'text-brand-700', bg: 'bg-brand-600', advice: 'Consider increasing calorie intake with nutrient-dense foods.' },
  ...
  return (
    <button
      ...
      className={`shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${
        active
          ? 'bg-brand-700 text-white shadow-lg shadow-brand-200 dark:shadow-none'
          : 'text-sand-500 dark:text-sand-400 hover:bg-sand-100 dark:hover:bg-brand-800'
      }`}
    >
    ...
    <div className="grid md:grid-cols-2 gap-6">
      ...
      <div className="bg-white dark:bg-brand-900 rounded-2xl border border-sand-100 dark:border-brand-800 p-6">
        <h2 className="font-bold text-sand-900 dark:text-white mb-5 flex items-center gap-2">
          <Activity className="w-5 h-5 text-brand-700" /> BMI Calculator
        </h2>

        {/* Unit toggle */}
        <div className="flex rounded-xl overflow-hidden border border-sand-200 dark:border-brand-700 mb-5">
          {['metric', 'imperial'].map(u => (
            <button key={u} onClick={() => setUnit(u)}
              className={`flex-1 py-2 text-sm font-semibold capitalize transition-colors ${
                unit === u ? 'bg-brand-700 text-white' : 'text-sand-500 dark:text-sand-400 hover:bg-sand-50 dark:hover:bg-brand-800'
              }`}>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-sand-500 dark:text-sand-400 uppercase tracking-wide mb-1.5">
              Height ({unit === 'metric' ? 'cm' : 'inches'})
            </label>
            <input
              type="number" value={height} onChange={e => setHeight(e.target.value)}
              placeholder={unit === 'metric' ? 'e.g. 170' : 'e.g. 67'}
              className="w-full px-4 py-3 rounded-xl border border-sand-200 dark:border-brand-700 bg-sand-50 dark:bg-brand-800 text-sand-900 dark:text-white placeholder-sand-400 text-sm focus:outline-none focus:ring-2 focus:ring-brand-600"
            />
          </div>
          <div>
            <label className="block text-xs font-semibold text-sand-500 dark:text-sand-400 uppercase tracking-wide mb-1.5">
      ...
      {/* Result */}
      <div className="bg-white dark:bg-brand-900 rounded-2xl border border-sand-100 dark:border-brand-800 p-6">
        <h2 className="font-bold text-sand-900 dark:text-white mb-5">Your Result</h2>
          ...
          {result ? (
            <motion.div key="result" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              <div className="text-center mb-6">
                <p className="text-6xl font-black text-sand-900 dark:text-white mb-1">{result.bmi}</p>
                <p className={`text-lg font-bold ${result.band.color}`}>{result.band.label}</p>
              </div>

              {/* BMI gauge bar */}
              <div className="mb-5">
                <div className="h-3 rounded-full bg-gradient-to-r from-gold-300 via-green-400 via-amber-400 to-red-500 relative mb-1">
                  <motion.div
                    initial={{ left: 0 }}
                    animate={{ left: `${getBMIPercent(result.bmi)}%` }}
                    transition={{ type: 'spring', stiffness: 200, damping: 25 }}
                    className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-white border-2 border-sand-800 dark:border-white rounded-full shadow-lg"
              ...
              <div className={`${result.band.bg} bg-opacity-10 dark:bg-opacity-20 rounded-xl p-4 mb-5`}>
                <p className="text-sm text-sand-700 dark:text-sand-300">{result.band.advice}</p>
              </div>

              {/* Health tip */}
              <div className="bg-brand-50 dark:bg-brand-900/20 rounded-xl p-4 mb-5">
                <p className="text-xs font-semibold text-brand-700 dark:text-gold-300 uppercase tracking-wide mb-2">Daily Health Tip</p>
            ...
            <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
              <Activity className="w-14 h-14 text-sand-200 dark:text-brand-700 mx-auto mb-3" />
              <p className="text-sand-400 dark:text-brand-600 font-medium">Enter your measurements</p>
              <p className="text-sm text-sand-300 dark:text-brand-700 mt-1">to see your BMI result</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
    ...
    <div>
      ...
      <div className="grid md:grid-cols-3 gap-5">
        {Object.entries(MEAL_PLAN).map(([meal, items]) => {
          ...
          return (
            <div key={meal} className="bg-white dark:bg-brand-900 rounded-2xl border border-sand-100 dark:border-brand-800 p-5">
              <h3 className="font-bold text-sand-900 dark:text-white mb-4">
                {icons[meal]} {meal}
              </h3>
              <div className="space-y-3">
                {items.map((item, i) => (
                  <div key={i} className="flex items-center justify-between py-2 border-b border-sand-50 dark:border-brand-800 last:border-0">
                    <p className="text-sm text-sand-700 dark:text-sand-300">{item.meal}</p>
                    <span className="text-xs font-semibold bg-sand-100 dark:bg-brand-800 text-sand-500 dark:text-sand-400 px-2 py-0.5 rounded-full">
                      {item.calories} kcal
                    </span>
                  </div>
                ))}
      ...
      <div className="bg-white dark:bg-brand-900 rounded-2xl border border-sand-100 dark:border-brand-800 p-6">
        ...
        {meds.length === 0 ? (
          <div className="text-center py-12">
            <Pill className="w-12 h-12 text-sand-200 dark:text-brand-700 mx-auto mb-3" />
            <p className="text-sand-400 font-medium">No medications added</p>
            <p className="text-sm text-sand-300 dark:text-brand-700 mt-1">Add your first medication →</p>
          </div>
        ) : (
          <div className="space-y-3">
            <AnimatePresence>
                ...
                <motion.div key={med.id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 12 }}
                  className={`flex items-center gap-3 p-4 rounded-xl border transition-all ${
                    med.takenToday
                      ...
                      : 'bg-sand-50 dark:bg-brand-800 border-sand-100 dark:border-brand-700'
                  }`}>
                  <button onClick={() => toggleTaken(med.id)}
                    className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors shrink-0 ${
                      med.takenToday ? 'bg-green-500 border-green-500 text-white' : 'border-sand-300 dark:border-brand-600'
                    }`}>
                    {med.takenToday && <Check className="w-3.5 h-3.5" />}
                  </button>
                  <div className="flex-1 min-w-0">
                    <p className={`font-medium text-sm ${med.takenToday ? 'line-through text-sand-400' : 'text-sand-900 dark:text-white'}`}>
                      {med.name}
                    </p>
                  ...
                  <button onClick={() => remove(med.id)} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-sand-400 hover:text-red-500 transition-colors">
                    <X className="w-4 h-4" />
                  </button>
                </motion.div>
              ))}
            ...
            {meds.length > 0 && (
              <p className="text-xs text-center text-sand-400 pt-2">
...
function HistoryTab({ currentUser }) {
  ...
  const trend = (i) => {
    ...
    if (Math.abs(diff) < 0.1) return <Minus className="w-4 h-4 text-sand-400" />;
    return diff > 0
  ...
  if (!currentUser) return (
    <div className="text-center py-20 bg-white dark:bg-brand-900 rounded-2xl border border-sand-100 dark:border-brand-800">
      <History className="w-12 h-12 text-sand-200 dark:text-brand-700 mx-auto mb-3" />
      <p className="font-medium text-sand-500">Sign in to view history</p>
    </div>
  );

  if (loading) return <div className="space-y-3">{[1,2,3].map(i => <div key={i} className="h-20 rounded-2xl bg-sand-200 dark:bg-brand-800 animate-pulse" />)}</div>;

  if (records.length === 0) return (
    <div className="text-center py-20 bg-white dark:bg-brand-900 rounded-2xl border border-sand-100 dark:border-brand-800">
      <History className="w-12 h-12 text-sand-200 dark:text-brand-700 mx-auto mb-3" />
      <p className="font-medium text-sand-500">No history yet</p>
      <p className="text-sm text-sand-400 mt-1">Calculate and save your BMI to track progress</p>
    </div>
  );
    ...
    <div>
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          <div key={stat.label} className="bg-white dark:bg-brand-900 rounded-2xl border border-sand-100 dark:border-brand-800 p-4 text-center">
            <p className="text-xl font-black text-sand-900 dark:text-white">{stat.value}</p>
            <p className="text-xs text-sand-400 mt-1">{stat.label}</p>
          </div>
        ))}
      </div>
      <div className="space-y-3">
        {records.map((r, i) => {
          ...
          return (
            <motion.div key={r.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              className="bg-white dark:bg-brand-900 rounded-2xl border border-sand-100 dark:border-brand-800 p-4 flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-sand-400 mt-0.5">{r.weight}{r.unit === 'metric' ? 'kg' : 'lbs'} · {r.height}{r.unit === 'metric' ? 'cm' : 'in'}</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-sand-400">{date.toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
  ...
  return (
    <PageTransition>
      <div className="min-h-screen bg-sand-50 dark:bg-brand-950 pt-16 pb-24 md:pb-8">
        {/* Header */}
        <div className="bg-gradient-to-r from-brand-700 to-brand-700 text-white py-8 px-4">
          <div className="max-w-5xl mx-auto">
            <h1 className="text-2xl font-bold flex items-center gap-2">
              <Activity className="w-6 h-6" /> Health Tracker
            </h1>
            <p className="text-brand-100 text-sm mt-1">BMI · Meal Planning · Medication Reminders · Progress History</p>
          </div>
        </div>

        <div className="max-w-5xl mx-auto px-4 py-6">
          {/* Tabs */}
          <div className="flex gap-2 mb-6 bg-white dark:bg-brand-900 rounded-2xl p-2 border border-sand-100 dark:border-brand-800 overflow-x-auto">
            {tabs.map(t => (
